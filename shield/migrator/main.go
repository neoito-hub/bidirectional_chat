...
func Migrate(db *gorm.DB) {
	...
	newAppPermissionErr := db.Exec(`
		...
		select a.app_id,p.permission_id,p.mandatory,now(),null from shield_apps a inner join permissions p on true where a.client_id='test-app-1' on conflict do nothing;
		`)
